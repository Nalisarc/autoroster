...
def main():
    ...
    if path is None:
        ...
        pass
    wb = autoroster.make_workbook()
    exam_exports = autoroster.read_in_roster(path)
    exam_list = autoroster.get_exam_list(exam_exports)
    exams_to_process = prompt_for_exams(exam_list)
    for exam in exams_to_process:
        student_info = autoroster.get_student_info(exam,exam_exports)
        autoroster.make_sheet(wb,exam,date,student_info)

    sheet_to_delete = wb.get_sheet_by_name('Sheet')
    wb.remove_sheet(sheet_to_delete)    
    outpath = get_outpath()
    autoroster.save_workbook(wb, outpath)
    return None
